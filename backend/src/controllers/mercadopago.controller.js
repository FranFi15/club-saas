...
function oauthCallbackHtml(ok, message) {
    ...
    const webUrl = oauthReturnUrl(ok);
    const nativeUrl = appDeepLink(ok ? 'mp-oauth/success' : 'mp-oauth/error');
    const safeWebLink = escapeHtml(webUrl);
...
<p style="margin-top:20px;"><a href="${safeNativeLink}" style="display:inline-block;padding:12px 20px;background:#009EE3;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;">Abrir Hermes Club App</a></p>
<p style="margin-top:12px;font-size:14px;"><a href="${safeWebLink}" style="color:#009EE3;">Continuar en el navegador</a></p>
<script>
(function(){
  var nativeUrl = ${JSON.stringify(nativeUrl)};
  var webUrl = ${JSON.stringify(webUrl)};
  var mobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent || '');
  setTimeout(function(){ window.location.href = mobile ? nativeUrl : webUrl; }, 600);
})();
</script>
</body></html>`;
}
