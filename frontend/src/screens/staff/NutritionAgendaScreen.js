...
/** Agenda de consultas nutricionales individuales (`consulta_nutricion`). */
export default function NutritionAgendaScreen({ navigation }) {
  const { clubData } = useContext(ClubContext);
  const { theme, isDarkMode } = useContext(ThemeContext);
  const colorMarca = clubData?.primaryColor || '#3b82f6';
  const agendaCacheKey = clubData?.urlIdentifier ? `nutri-agenda:${clubData.urlIdentifier}` : '';
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={['top']}>
      ...
      <CoachScreenHeader
        ...
        kicker="Consultas"
        title="Mis citas"
        subtitle="Individual por atleta · lugar libre (consultorio, domicilio, videollamada…)"
        rightAccessory={<CoachHeaderFab colorMarca={colorMarca} onPress={() => navigation.navigate('NutritionNewConsult')} />}
      />
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={[styles.empty, { color: theme.textMuted }]}>
              No hay consultas programadas. Creá una con el botón + (elegí categoría, atleta y lugar).
            </Text>
          }
        />
      )}
