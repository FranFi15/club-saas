...
export default function NutritionDashboardScreen({ navigation }) {
  ...
  const dashboardCacheKey = clubData?.urlIdentifier ? `nutri-dashboard:${clubData.urlIdentifier}` : '';
  ...
  const fetchAgenda = useCallback(async () => {
    ...
    const res = await clubApi.get('/sessions/nutricionista/agenda', { headers: h });
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={['top']}>
      ...
      >
        <Text style={[styles.sectionLabel, { color: theme.textMuted }]}>Próximas consultas</Text>
        {showInitialLoader ? (
          ...
          <View style={[styles.empty, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.emptyTxt, { color: theme.textMuted }]}>
              No tenés citas programadas. Creá una desde Sesiones (un atleta por vez y lugar en texto).
            </Text>
          </View>
        ) : (
          consultas.map((s) => {
        ...
        <Card
          ...
          subtitle="Consultas individuales con lugar libre y cierre cuando terminen"
          onPress={() => tabNav()?.navigate('NutSesiones', { screen: 'NutritionAgenda' })}
        />
        <Card
          icon="people-outline"
          title="Mis atletas"
          subtitle="Listado de atletas, mediciones y wellness"
          onPress={() => tabNav()?.navigate('NutEquipo', { screen: 'NutRoster' })}
        />
        <Card
          icon="chatbubbles-outline"
          title="Comunicar"
          subtitle="Noticias, material multimedia y pedidos de documentación"
          onPress={() => tabNav()?.navigate('NutComunicar', { screen: 'CoachCommsHub' })}
        />
      </ScrollView>
    </SafeAreaView>
  );
