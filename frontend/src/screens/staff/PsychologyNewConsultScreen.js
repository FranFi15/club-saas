...
/** Alta de sesión tipo `consulta_psicologia`: un atleta y lugar en texto libre. */
export default function PsychologyNewConsultScreen({ navigation }) {
  const { clubData } = useContext(ClubContext);
  const { theme, isDarkMode } = useContext(ThemeContext);
  const colorMarca = clubData?.primaryColor || '#3b82f6';
  const metaCacheKey = clubData?.urlIdentifier ? `psychology-new-consult:${clubData.urlIdentifier}` : '';
        ...
        >
          ...
          >
            ...
            <TextInput
              ...
              placeholder="Ej. Consultorio · Videollamada · Sala club"
            ...
            <CalendarDateField
              ...
              value={fecha}
              onChange={setFecha}
              placeholder="Elegí el día de la sesión"
            />
            ...
            <TouchableOpacity
              ...
              disabled={saving}
            >
              <Text style={styles.primaryBtnTxt}>{saving ? 'Guardando…' : 'Crear sesión'}</Text>
            </TouchableOpacity>
          </ScrollView>
        </KeyboardAvoidingView>
      )}
