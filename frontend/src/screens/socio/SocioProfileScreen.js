...
export default function SocioProfileScreen({ navigation }) {
  const { clubData, setClubData, clearSession } = useContext(ClubContext);
  const { theme, isDarkMode } = useContext(ThemeContext);
  const { profile, refresh } = useMember();
  const { tab } = useBadges();
  const colorMarca = clubData?.primaryColor || '#3b82f6';
  const profileCacheKey = clubData?.urlIdentifier ? `socio-profile-view:${clubData.urlIdentifier}` : '';
      ...
      <CoachScreenHeader
        ...
        footer={
          <CoachHeaderBadge>
            ...
            <Text style={styles.heroBadgeTxt}>Socio</Text>
          </CoachHeaderBadge>
        }
      />
      ...
      >
        ...
        <View style={[profileCardStyles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          <ProfileLinkRow
            icon="wallet-outline"
            title="Cuota social"
            subtitle="Ver y pagar tu cuota social"
